...
neural_network::neural_network(){
	...
	for(int i=0;i<TRAINING_INPUTS+1;i++){
...
void neural_network::iterate(){
	...
	for(int i=0;i<TRAINING_RECORDS;i++){
		for(int j=0;j<TRAINING_INPUTS;j++){
			onode.in_data[j]=training_set[i][j];
			onode.weights[j]=weights[j];
		}
		onode.in_data[TRAINING_INPUTS]=bias;
...
void output_node::node_function(){
	double output=0;
...
void neural_network::modify_weights(int i){
	for(int j=0;j<TRAINING_INPUTS;j++){
		weights[j]+=errors[i]*training_set[i][j];
	}
	weights[TRAINING_INPUTS]+=errors[i]*bias;
}

void neural_network::print(int i){
	
	printf("|%2d\t%2d |\t%5.2f\t%5.2f\t%5.2f |\t%2d\t%2d\t%5.2f |\t%5.2f\t%5.2f\t%5.2f|\n",
		   training_set[i][0],training_set[i][1],bias*weights[2],training_set[i][0]*weights[0],training_set[i][1]*weights[1],
